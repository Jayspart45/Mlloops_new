...
from singularity_loop.core.utils.windows_sqlite_fix import windows_dll_fix
windows_dll_fix()
...
from singularity_loop.core.argparser import parse_input_args
from singularity_loop.core.utils.params import get_env
...
def _setup_env():
    ...
    os.environ.setdefault("DJANGO_SETTINGS_MODULE", "singularity_loop.core.settings.singularity_loop")
...
def main():
    ...
    from singularity_loop.core.utils.common import collect_versions
    ...
    if input_args.command == 'version' or input_args.version:
        from singularity_loop import __version__
    ...
    # init
        ...
        if input_args.command != 'start':
            print('Start the server: singularity-loop start ' + input_args.project_name)
            return

    # start with migrations from old projects, '.' project_name means 'singularity-loop start' without project name
    elif input_args.command == 'start' and input_args.project_name != '.':
        from singularity_loop.core.old_ls_migration import migrate_existing_project
        from projects.models import Project
        sampling_map = {'sequential': Project.SEQUENCE, 'uniform': Project.UNIFORM,
                        'prediction-score-min': Project.UNCERTAINTY}
        ...
        if not _project_exists(input_args.project_name):
            ...
            if project_path.exists():
                print('Project directory from previous verion of singularity-loop found')
            ...
            if not migrated:
                print(
                    ...
                    'Did you miss create it first with `singularity-loop init {project_name}` ?'.format(
    ...
    if input_args.command == 'start' or input_args.command is None:
        from singularity_loop.core.utils.common import start_browser
